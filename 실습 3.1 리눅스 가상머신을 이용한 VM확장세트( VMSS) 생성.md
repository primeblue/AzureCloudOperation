...
1. Azure 포털의 검색창에서 Virtual Machine Scale Set 을 검색 하여 가상 머신 확장집합 만들기를 클릭한다. 
   ...
   7. 웹서비스를 위한  http 80 포트와 ssh 22 포트를 인바운드 보안 규칙에 허용인지 확인해본다. 

      ![image-20200715041858878](/1_3_16.png)

   8.ssh TCP 22 과  HTTP TCP 80 에 대한 인바운트 허용 규칙이 없다면 인바운드 포트 규칙 추가를 클릭하여 규칙을 추가해준다.

      ![image-20200715042056750](/1_3_17.png)
      
   9. 부하분산장치(LB) 을 클릭하여 인바운트 NAT 규칙을 클릭하여 로드발런서의 백엔드 서버들과 연결해주는 서비스포트넘버를 확인해본다.
      ![image-1_3_18](/1_3_18.png)
      
  10. ssh azureadmin@VMSS_Public_IP -p 50001  
      ssh azureadmin@VMSS_Public_IP -p 50002
      로 각각의 백엔드 서버에 접속도 가능하다.
