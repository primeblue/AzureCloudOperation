...
8. 다음 명령을 수행하여  생성한 리눅스 가상서버에 SSH 접속이 가능하도록 22번 포트를 열어준다.
   ...
   ```
   az vm open-port --port 22 --resource-group uer00RG001 --name user00VM01 --priority 100
   az vm open-port --port 80 --resource-group user00RG001 --name user00VM01 --priority 110
   ```
